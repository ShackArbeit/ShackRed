<script setup>
import { showCellPhoneStore } from '../../stores/showCellPhone'
const showCellPhone=showCellPhoneStore()
const openShowMainPart = ()=>{
      showCellPhone.openShowMainPart()
      console.log('再次確認已經打開了')
}


</script>


<template>
<button class="container" @click="openShowMainPart"> 
   預覽
</button>
<div class="eyeButton" @click="openShowMainPart">
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 18 18" fill="none">
        <mask id="mask0_59266_6078" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
        <rect width="24" height="24" fill="#D9D9D9"/>
        </mask>
        <g mask="url(#mask0_59266_6078)">
        <path d="M9 12C9.9375 12 10.7344 11.6719 11.3906 11.0156C12.0469 10.3594 12.375 9.5625 12.375 8.625C12.375 7.6875 12.0469 6.89062 11.3906 6.23438C10.7344 5.57812 9.9375 5.25 9 5.25C8.0625 5.25 7.26562 5.57812 6.60938 6.23438C5.95312 6.89062 5.625 7.6875 5.625 8.625C5.625 9.5625 5.95312 10.3594 6.60938 11.0156C7.26562 11.6719 8.0625 12 9 12ZM9 10.65C8.4375 10.65 7.95938 10.4531 7.56563 10.0594C7.17188 9.66563 6.975 9.1875 6.975 8.625C6.975 8.0625 7.17188 7.58438 7.56563 7.19063C7.95938 6.79688 8.4375 6.6 9 6.6C9.5625 6.6 10.0406 6.79688 10.4344 7.19063C10.8281 7.58438 11.025 8.0625 11.025 8.625C11.025 9.1875 10.8281 9.66563 10.4344 10.0594C10.0406 10.4531 9.5625 10.65 9 10.65ZM9 14.25C7.175 14.25 5.5125 13.7406 4.0125 12.7219C2.5125 11.7031 1.425 10.3375 0.75 8.625C1.425 6.9125 2.5125 5.54688 4.0125 4.52813C5.5125 3.50938 7.175 3 9 3C10.825 3 12.4875 3.50938 13.9875 4.52813C15.4875 5.54688 16.575 6.9125 17.25 8.625C16.575 10.3375 15.4875 11.7031 13.9875 12.7219C12.4875 13.7406 10.825 14.25 9 14.25ZM9 12.75C10.4125 12.75 11.7094 12.3781 12.8906 11.6344C14.0719 10.8906 14.975 9.8875 15.6 8.625C14.975 7.3625 14.0719 6.35938 12.8906 5.61562C11.7094 4.87187 10.4125 4.5 9 4.5C7.5875 4.5 6.29063 4.87187 5.10938 5.61562C3.92812 6.35938 3.025 7.3625 2.4 8.625C3.025 9.8875 3.92812 10.8906 5.10938 11.6344C6.29063 12.3781 7.5875 12.75 9 12.75Z" fill="white"/>
        </g>
    </svg>
</div>

</template>


<style scoped>
.container{
    height:48px;
    width:21.59%; 
    color:#ffffff;
    border-radius: 100px;
    background:  #3A3131;
    font-size:24px;
    cursor: pointer;
}

@media(min-width:1200px){
    .eyeButton{
         display:none;
    }
}
@media(max-width:1200px){
    .eyeButton{
         display:none;
    }
}

@media(max-width:768px){
    .container{
        display:none;
    }
    .eyeButton{
        display: flex;
        width: 48px;
        height: 48px;
        max-width: 120px;
        justify-content: center;
        align-items: center;
        border-radius: 100px;
        background:  #635D5D;
    }
}
</style>